...
/**
 * Setter for the boost rendering state.<br/><b>null</b> (default) turns boost on automatically, but only when the series has more points than {@link anychart.charts.Cartesian#boostThreshold} <b>and</b> it is the only enabled series on the plot. <b>true</b> boosts unconditionally, bypassing both of those checks. <b>false</b> never boosts.<br/><b>Note</b>: Boost never applies to stacked, 3D, OHLC or range-based series, whatever this option is set to. Boost draws on a Canvas layer; the WebGL renderer is an optional module loaded with anychart.loadModule('boost-webgl') and falls back to Canvas when it is not present.
