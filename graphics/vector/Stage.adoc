 ...
 * Suspended rendering is much faster than instant, when there are a lot of operations with Stage.<br/>
 * This method calculates the count of calls and allows the inserted constructions correctly
 ...
 * @detailed This method calculates the count of calls and allows the inserted constructions correctly
 * (suspend(); suspend(); resume(); resume() - rendering will happens on the last resume());
 ...
 * STAGE_RENDERED event on the stage. To control the suspend of rendering,
