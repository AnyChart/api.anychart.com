 ...
 * @detailed This method works only if {@link anychart.scales.Linear#stackMode} method has not "none" value.<br/>
 * The {@link anychart.charts.StreamGraph} is the exception: its layers are always stacked, so the direction applies whatever the stack mode is, "none" included.
 ...
 * @example anychart.scales.Linear.maximum
