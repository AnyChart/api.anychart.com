 ...
 * @detailed <ul>
 *   <li><b>'float':</b> both lines follow the pointer. The default for Scatter charts.</li>
 *   <li><b>'sticky':</b> both lines jump from one data point to the next. The default for Stock charts.</li>
 *   <li><b>'sticky-x':</b> the vertical line jumps from one data point to the next, the horizontal line and its Y label follow the pointer. Supported by Stock plots only; other chart types act as 'float'.</li>
 * </ul>
 * Pass the value as a string.
 * @listing See listing
 * chart.plot(0).crosshair().displayMode('sticky-x');
 * @example anychart.core.ui.Crosshair.displayMode_set
 * @param {(anychart.enums.CrosshairDisplayMode|string)=} opt_mode Display mode.
 * @return {anychart.core.ui.Crosshair} Self instance for method chaining.
 * @since 7.7.0
 */
anychart.core.ui.Crosshair.prototype.displayMode;
