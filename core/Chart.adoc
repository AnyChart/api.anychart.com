 ...
 * @param {Object.<string, (string|boolean|undefined|csvSettingsFunction|Object)>=} opt_csvSettings CSV settings.
 * CSV settings object has fields like:
 ...
 *  formats - a function with two arguments such as the field name and value, that returns the formatted value,
 *  the object with the key as the field name, and the value as a format function. Default is 'undefined'.
